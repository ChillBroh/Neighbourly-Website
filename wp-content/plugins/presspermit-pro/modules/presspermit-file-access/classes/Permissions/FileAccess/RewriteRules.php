...
class RewriteRules
{
    ...
    static function &buildSiteFileRules($args = [])
    {
        ...
        if ($pass_small_thumbs = presspermit()->getOption('small_thumbnails_unfiltered')) {
            ...
            $thumb_filtered_ids = $wpdb->get_col("SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_pp_file_filtering' AND meta_value = 'all' AND post_id IN ('$thumbnail_ids_csv')");
        } else {
            $thumb_filtered_ids = [];
        }
