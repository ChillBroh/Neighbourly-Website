...
class Triggers
{
    ...
    */
    ...
    {
        if (!presspermit_empty_REQUEST('found_post_id') && presspermit_is_REQUEST('media')) {
    ...
    {
        if (presspermit_is_GET('action', 'presspermit-expire-file-rules')) {
            require_once(PRESSPERMIT_FILEACCESS_CLASSPATH . '/RuleFlush.php');
            RuleFlush::requestedFileRuleFlush();

        } elseif (presspermit_is_GET('action', 'presspermit-attachment-utility')) {
