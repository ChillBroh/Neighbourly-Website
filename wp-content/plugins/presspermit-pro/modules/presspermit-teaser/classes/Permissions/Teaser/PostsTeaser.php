...
class PostsTeaser
{
    ...
    function postsTeaserPrepResults($results, $tease_otypes, $args = '')
    {
        ...
        if (!empty($query_obj->query_vars['posts_per_page'])) {
            $this->buffer_found_posts = $wpdb->get_var(apply_filters_ref_array('found_posts_query', ['SELECT FOUND_ROWS()', &$query_obj]));
        ...
        $this->readable_posts[$rkey] = apply_filters('presspermit_teaser_readable_posts', $wpdb->get_col($filtered_request), $request, $filtered_request, ['object_types' => $object_type, 'post_ids' => $post_ids]);
        $hide_custom_private = [];
    ...
    public function fltLimitPostIDs($where, $clauses, $args)
    {
        if (!empty($args['post_ids'])) {
            ...
            if (!defined('PRESSPERMIT_TEASER_NO_POSTIDS_SAFEGUARD')) {
