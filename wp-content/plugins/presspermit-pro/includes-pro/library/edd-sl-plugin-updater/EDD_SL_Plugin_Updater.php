...
class EDD_SL_Plugin_Updater {
    ...
    public function show_update_notification( $file, $plugin ) {
        ...
        if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version, $version_info->new_version, '<' ) ) {
            ...
            echo '<tr class="plugin-update-tr" id="' . esc_attr($this->slug) . '-update" data-slug="' . esc_attr($this->slug) . '" data-plugin="' . esc_url($this->slug . '/' . $file) . '">';
    ...
    public function show_changelog() {
        ...
        if (!presspermit_is_REQUEST('edd_sl_action', 'view_plugin_changelog')) {
            return;
        }

        if (!$plugin = presspermit_REQUEST_key('plugin')) {
            return;
        }

        if (!$slug = presspermit_REQUEST_key('slug')) {
            return;
        }
        ...
        $cache_key    = md5( 'edd_plugin_' . $plugin . '_' . $beta . '_version_info' );
